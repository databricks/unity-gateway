...
class TestRenderOverlay:
    ...
    def test_managed_header_value_with_newline_is_dropped(self):
        # FIX 1: A managed header value containing a newline should be dropped to prevent
...
class TestDiscoveryMarkerTiming:
    ...
    def test_discovery_run_with_reconcile_success_clears_markers(self, monkeypatch):
        # FIX 2c & FIX A: When a DISCOVERY run (no static picker) has a SUCCESSFUL reconcile,
        # the markers should be cleared and persisted AFTER reconcile (only if the file was written).
        private_writes: list = []
        managed_writes: list = []
        state_saves: list = []
        self._patch(monkeypatch, private_writes, managed_writes)
    ...
    def test_discovery_run_with_suppressed_managed_write_preserves_markers(self, monkeypatch):
        # FIX A: When a discovery run (no static picker) has a SUPPRESSED managed write
