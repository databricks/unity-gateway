...
# The raw CodingAgentConfig as the gateway returns it (what managed-config.json stores).
...
FULL_RAW_CONFIG = {
    ...
    "mcp_servers": {"names": ["system.ai.slack"]},
...
def _isolate_settings(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(managed_config_mod, "MANAGED_CONFIG_PATH", tmp_path / "managed-config.json")
...
class TestBuildPayload:
    def test_excludes_server_owned_metadata(self):
        ...
        assert payload["mcp_servers"] == {"names": ["system.ai.slack"]}
    ...
    def test_export_spec_version_wins_over_raw_config_spec_version(self):
        # A raw config carrying its own spec_version must not override the export envelope's
        # EXPORT_SPEC_VERSION: the server-owned field is stripped before the spread.
        raw = {**FULL_RAW_CONFIG, "spec_version": 99}
        with _with_config(raw):
            payload = export_mod.build_export_payload()
        assert payload["spec_version"] == export_mod.EXPORT_SPEC_VERSION
