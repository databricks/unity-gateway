...
class TestDiscoverClaudeModels:
    ...
    def test_paginates_anthropic_catalog_and_preserves_metadata_and_scope(
        self, monkeypatch, scope_kwargs, expected_headers
    ):
        pages = {
            None: {
        ...
        assert catalog.model_id_to_description == {
            "claude-first": "First description",
            "claude-second": "Second description",
        }
        assert catalog.error_msg is None
        assert [request[0] for request in requests] == [
            f"{WS}/ai-gateway/anthropic/v1/models",
            f"{WS}/ai-gateway/anthropic/v1/models?after_id=cursor%2Fone%3Ftwo",
        ]
        ...
        [
            (
                [
                    ...
                    {"data": [{"id": "claude-second"}], "has_more": True, "last_id": "same"},
                ],
    ...
    def test_rejects_invalid_anthropic_pagination_cursor(self, monkeypatch, pages, expected_error):
        ...
        def fake_get(url, token, **kwargs):
            requests.append(url)
            return pages[min(len(requests) - 1, len(pages) - 1)], None
    ...
    def test_rejects_anthropic_page_fetch_failure_without_partial_success(self, monkeypatch):
        ...
        assert catalog.error_msg == (
            "failed to fetch Anthropic model catalog page 2: HTTP 503 Service Unavailable"
        )
        assert calls[1][0] == f"{WS}/ai-gateway/anthropic/v1/models?after_id=next"

    def test_bounds_anthropic_catalog_pagination(self, monkeypatch):
        calls = []
        ...
        def fake_get(url, token, **kwargs):
            ...
            return {
                "data": [{"id": f"claude-{page_number}"}],
                "has_more": True,
                "last_id": f"cursor-{page_number}",
            }, None
        ...
        assert catalog.error_msg == (
            ...
            f"{db_mod._ANTHROPIC_MODEL_DISCOVERY_MAX_PAGES} pages"
        )
        assert len(calls) == db_mod._ANTHROPIC_MODEL_DISCOVERY_MAX_PAGES
    ...
    @pytest.mark.parametrize(
        ...
        ids=["non-object", "non-list-data", "non-object-model"],
    )
    def test_rejects_malformed_anthropic_page(self, monkeypatch, payload):
        monkeypatch.setattr(db_mod, "_http_get_json", lambda *args, **kwargs: (payload, None))

        catalog = db_mod.list_anthropic_model_catalog(WS, "token")

        assert catalog.model_ids == []
        assert catalog.error_msg is not None
        assert "Anthropic model catalog page 1" in catalog.error_msg
