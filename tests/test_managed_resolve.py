...
class TestManagedSuppliesModels:
    ...
    def test_false_for_codex_with_only_model_service_location(self):
        # FIX B: Codex does NOT read model_service_location, so location-only config
        # should return False to allow discovery to run normally. Claude returns True for
        # location-only config because it reads it, but codex must still discover.
        managed = {
            "enabled_agents": {
                "codex": {"model_config": {"model_service_location": "main.catalog.models"}}
            }
        }
    ...
    def test_true_for_claude_with_only_model_service_location(self):
        ...
        managed = {
            "enabled_agents": {
                "claude": {"model_config": {"model_service_location": "main.catalog.models"}}
            }
        }
        assert managed_supplies_models(managed, "claude") is True

    def test_true_for_codex_with_static_models_and_location(self):
        # Codex with static models (names) should return True even if location is also set,
        # since static models take precedence.
        managed = {
            "enabled_agents": {
                "codex": {
                    "model_config": {
                        "names": ["gpt-4o"],
                        "model_service_location": "main.catalog.models",
                    }
                }
            }
        }
...
class TestStaticAndAutoModels:
    ...
    def test_names_and_location_count_as_supplying_models(self):
        ...
        # FIX B: Codex does NOT read model_service_location, so it should return False
        # even when location is set. Claude returns True for location-only config.
        assert (
            managed_supplies_models(
        ...
        # But claude with location should return True
        assert (
            managed_supplies_models(
                self._managed("claude", {"model_service_location": "system.ai"}), "claude"
            )
            is True
        )
...
class TestCustomHeaders:
    def test_managed_custom_headers_accessor_returns_dict(self):
        managed = {
            "enabled_agents": {
                "claude": {"custom_headers": {"X-My-Tag": "hello", "X-Other": "world"}}
            }
        }
    ...
    def test_managed_state_overrides_includes_custom_headers(self):
        managed = {"enabled_agents": {"claude": {"custom_headers": {"X-Custom": "test-value"}}}}
    ...
    def test_resolve_state_overlays_custom_headers(self):
        managed = {"enabled_agents": {"claude": {"custom_headers": {"X-Managed": "managed-value"}}}}
        state = _state()
        resolved = resolve_state(managed, state, "claude")
