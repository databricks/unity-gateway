...
class TestListConfiguredSkills:
    ...
    def captured(self, monkeypatch):
        ...
        def fake_render(headers, rows, *args, **kwargs):
            calls["headers"], calls["rows"] = headers, rows
            return ""

        monkeypatch.setattr(sd, "render_box_table", fake_render)
        monkeypatch.setattr(sd, "print_note", lambda msg: calls.__setitem__("note", msg))
        monkeypatch.setattr(sd, "load_state", lambda: {})
        monkeypatch.setattr(sd, "install_databricks_cli", lambda **kwargs: None)
    ...
    def _downloaded(self, monkeypatch, fqns):
        ...
        monkeypatch.setattr(sd, "list_downloaded", lambda: records)

    def _scoped(self, monkeypatch, scopes, by_schema):
        monkeypatch.setattr(
            sd, "configured_skill_scopes", lambda _state: (WS, scopes) if scopes else None
        )
        monkeypatch.setattr(
            sd,
            "list_schema_skills",
            lambda ws, tok, c, s: by_schema.get(f"{c}.{s}", ([], None)),
        )
    ...
    def test_downloaded_only_is_visible_to_all_agents(self, monkeypatch, captured):
        ...
        self._scoped(monkeypatch, {}, {})

        sd.list_configured_skills_command()
