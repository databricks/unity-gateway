"""CUJs: configure against a managed workspace and confirm the admin's static model list.

These run against the managed e2e workspace (`E2E_ADMIN_WORKSPACE`), which publishes a
CodingAgentConfig whose `model_services` are the static allow-lists below. They are the only
journeys that exercise the managed path end to end: `ug configure` applies the admin config to
every enabled agent without an agent selector, and each agent's generated config exposes exactly
the admin's list (Claude's `availableModels`/`modelPicker`, Codex's model catalog). The constants
mirror that published config; update them here if the admin list changes.
"""
...
def test_ug_configure_managed_claude(live_session, workspace):
    ...
    personal agent selector, Claude's generated settings expose exactly the admin's
    static model_services as its picker allow-list, and launching Claude reaches a real
    gateway prompt rather than the account-login flow.
    """
    session = live_session
    result = session.run("configure", "--workspace", workspace, "--skip-upgrade", timeout=240)
    # The admin dictates the setup: the personal agent selector never appears and the
    # managed confirmation is shown instead.
...
def test_ug_configure_managed_codex(live_session, workspace):
    ...
    Expected: ug applies the admin config to every enabled agent without showing the
    personal agent selector, Codex's generated model catalog lists exactly the admin's
    static model_services, and launching Codex reaches a real gateway prompt rather than
