...
publishes a CodingAgentConfig), **22 managed-fixture cases** (marker `managed_fixture`, with only
the CodingAgentConfig input injected), and **5 installation checks**. Twelve explicit Claude
journeys cover configured and fresh state across the managed-discovery matrix; ten existing
collected cases cover focused model, MCP, skills, and lifecycle shapes. Parametrization varies
