...
| `test_managed_fixture_claude_mps_defaults_accompany_discovery`, `test_managed_fixture_claude_parent_schema_defaults_accompany_discovery` | Launch Claude from injected managed defaults with MPS and Unity Catalog discovery | Both generated settings files retain every authored family default alongside the source header; live budget recommendations may choose launch-scoped `ANTHROPIC_MODEL`; only UC Opus/Sonnet family ids gain `[1m]` |
