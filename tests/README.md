...
| `test_smart_routing_claude_subagent_only_launch_shows_no_first_prompt_banner`, `test_smart_routing_codex_subagent_only_launch_records_usage` | Configure, then launch the real TUI with both the full and subagent-only routing flags set; Codex delegates its task to one native child | Subagent-only takes precedence with no first-prompt routing wrapper; Claude's SessionStart canary proves its routing hooks armed; Codex proves a routed native child completed and its real SubagentStop hook wrote a correlated token-usage CSV |
...
| Claude/Codex interactive smart routing | First-prompt routing covered by the `managed_fixture` smart-routing banner journeys; hook protocol covered by the route-subagent journeys against the live router; Codex's subagent-only launch requires a real routed child and verifies its native SubagentStop usage CSV. Claude's interactive spawn decision, interactive explicit-model bypass, and dedicated routing CI shards remain deferred. Unit/component routing tests do not establish live routing behavior. |
