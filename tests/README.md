...
| `test_case_02_*`, `test_case_04_*` | Launch managed Codex after configure and from fresh state, with personal discovery enabled and disabled | ug-launched and fresh bare Codex app servers expose exactly the admin MPS-scoped catalog through the stable catalog and shared config; GUI rendering is not covered |
