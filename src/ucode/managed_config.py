    ...
    if spec is not None:
        if isinstance(spec, bool) or not isinstance(spec, int):
            return FetchedManagedConfig(
                None,
                f"Your CLI Managed Configuration has an unrecognized spec_version ({spec!r}); "
                "update Unity Gateway with `ug upgrade`.",
            )
        if spec > MAX_SPEC_VERSION:
            return FetchedManagedConfig(
                None,
                f"Your CLI Managed Configuration needs a newer Unity Gateway (spec_version {spec}; "
...
def _persisted_fallback(workspace: str, reason: str, *, refused: bool = False) -> dict | None:
    ...
    if refused:
        print_warning(
            f"Your CLI Managed Configuration is not readable by you ({summary}); using the last "
            "one saved for this workspace. Ask an admin to grant access."
        )
    else:
        print_warning(
            f"Could not read your CLI Managed Configuration ({summary}); "
