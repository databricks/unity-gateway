...
def managed_custom_headers(managed: dict, tool: str) -> dict[str, str]:
    ...
    headers = _as_dict(_agent_entry(managed, tool).get("custom_headers"))
...
def managed_state_overrides(managed: dict, tool: str) -> dict[str, object]:
    ...
    if custom_hdrs:
        overrides[f"{tool}_custom_headers"] = custom_hdrs
    return overrides
...
def managed_supplies_models(managed: dict | None, tool: str) -> bool:
    ...
    if tool == "claude" and (
        ...
        return True
    if tool == "codex" and managed_static_models(managed or {}, tool):
        return True
